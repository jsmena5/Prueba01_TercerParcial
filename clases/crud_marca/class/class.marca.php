...
class marca{
	...
	public function get_form($idMarca=NULL){
        ...
        if($idMarca == NULL)  {
            ...
            $num = $res->num_rows;            
            if($num==0){
                $mensaje = "tratar de actualizar la marca con idMarca= ".$idMarca . "<br>";
...
//*************************************************************************************************        
    ...
    public function get_detail_marca($idMarca){
		$sql = "SELECT m.idMarca, m.descripcion
                FROM marca m
	...
	if($num == 0){
        $mensaje = "desplegar el detalle de la marca con idMarca= ".$idMarca . "<br>";
        echo $this->_message_error($mensaje);
...
public function delete_marca($idMarca){
	...
	public function get_list(){
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="text-secondary">Lista de Marcas</h3>
            <a href="index.php?d=' . $d_new_final . '" class="btn btn-success">Ingresa Marca</a>
        </div>
		...
		//if($num != 0){
		    ...
		    while($row = $res->fetch_assoc()){ 
				$d_del = "del/" . $row['idMarca']; //tengo el id del registro que yo escoja 
