...
class usuario{
	...
	public function get_form($idusuario=NULL){
        ...
        if($idusuario== NULL)  {
            ...
            $num = $res->num_rows;            
            if($num==0){
                $mensaje = "tratar de actualizar la marca con idMarca= ".$idusuario. "<br>";
                    ...
                    <form name="Form_Marca" method="POST" action="index.php" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="username"><strong>Descripción:</strong></label>
                                <input type="text" class="form-control" name="descripcion" value="' . $this->username .'" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="password"><strong>Descripción:</strong></label>
                                <input type="text" class="form-control" name="descripcion" value="' . $this->password .'" required>
                            </div>
                        </div>
...
//*************************************************************************************************        
    ...
    public function get_detail_usuario($idusuario){
		$sql = "SELECT u.idusuario, u.username, u.password
                FROM usuario u
	...
	if($num == 0){
        $mensaje = "desplegar el detalle de los usuarios con idusuario= ".$idusuario . "<br>";
        echo $this->_message_error($mensaje); 
...
public function delete_marca($idMarca){
	...
	public function get_list(){
		...
		//if($num != 0){
		    ...
		    while($row = $res->fetch_assoc()){ 
				//$d_del = "del/" . $row['idUsuario']; //tengo el id del registro que yo escoja 
